...
if (empty($_REQUEST['path']) || $_REQUEST['path'] == '/') { // проверяю на какую страницу отправляется запрос
...
} elseif ($_REQUEST['path'] == 'sign_up') { // если ссылка ведет на страницу регистрации
    include_once('../modules/sign_up.inc.php'); // подключаю модль регистрации
} elseif ($_REQUEST['path'] == 'logout') { // если пользователь нажал кнопку logout
    setcookie('session_hash', '', 0); // уничтожаю кукис
    header('Location: /'); // перенаправляю на главную
} elseif ($_REQUEST['path'] == 'profile') { // если пользователь перешел на страницу профиля
    include_once('../modules/profile.inc.php'); // подключаю модуль профиля
} elseif ($_REQUEST['path'] == 'delete') { // если пользователь перешел на страницу профиля
    include_once('../modules/delete.inc.php'); // подключаю модуль профиля
}
